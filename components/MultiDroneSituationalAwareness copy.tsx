...
// ============================================================================
// HELICOPTER ICON FOR MISSION DRONE
// ============================================================================
  ...
  const helicopterSVG = `
    <div style="width: ${size}px; height: ${size}px; position: relative; transform: rotate(${heading}deg);">
      <svg width="${size}" height="${size}" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <!-- Main Rotor (Top) -->
        <ellipse cx="24" cy="12" rx="18" ry="3" fill="${color}" opacity="0.4"/>
        <line x1="6" y1="12" x2="42" y2="12" stroke="${color}" stroke-width="2" opacity="0.6"/>
        
        <!-- Rotor Hub -->
        <circle cx="24" cy="12" r="3" fill="${color}" stroke="white" stroke-width="1.5"/>
        
        <!-- Rotor Mast -->
        <rect x="23" y="12" width="2" height="8" fill="${color}"/>
        
        <!-- Main Body (Fuselage) -->
        <ellipse cx="24" cy="26" rx="8" ry="10" fill="${color}" stroke="white" stroke-width="2"/>
        
        <!-- Cockpit Window -->
        <ellipse cx="24" cy="22" rx="5" ry="4" fill="white" opacity="0.8" stroke="${color}" stroke-width="1"/>
        <ellipse cx="24" cy="22" rx="3" ry="2.5" fill="${color}" opacity="0.3"/>
        
        <!-- Tail Boom -->
        <rect x="32" y="25" width="12" height="3" rx="1.5" fill="${color}" stroke="white" stroke-width="1.5"/>
        
        <!-- Tail Rotor -->
        <circle cx="44" cy="26.5" r="3.5" fill="${color}" stroke="white" stroke-width="1.5" opacity="0.7"/>
        <line x1="44" y1="23" x2="44" y2="30" stroke="white" stroke-width="1" opacity="0.8"/>
        <line x1="40.5" y1="26.5" x2="47.5" y2="26.5" stroke="white" stroke-width="1" opacity="0.8"/>
        
        <!-- Landing Skids -->
        <path d="M 16 34 Q 16 37 18 38 L 30 38 Q 32 37 32 34" 
              stroke="${color}" stroke-width="2.5" fill="none" stroke-linecap="round"/>
        <line x1="18" y1="32" x2="18" y2="38" stroke="${color}" stroke-width="2"/>
        <line x1="30" y1="32" x2="30" y2="38" stroke="${color}" stroke-width="2"/>
        
        <!-- Side Windows -->
        <ellipse cx="20" cy="26" rx="2" ry="3" fill="white" opacity="0.4"/>
        <ellipse cx="28" cy="26" rx="2" ry="3" fill="white" opacity="0.4"/>
        
        <!-- Direction Indicator (nose) -->
        <circle cx="24" cy="18" r="2" fill="white"/>
        
        <!-- Shadow effect -->
        <ellipse cx="24" cy="39" rx="10" ry="2" fill="black" opacity="0.2"/>
      </svg>
    </div>
  `;
  ...
  return (
    <div className="relative h-full w-full bg-slate-900">
      ...
      >
              ...
              <div className="p-3 min-w-[220px]">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-600">Mission Helicopter</div>
                  </div>
        ...
        {/* LIVE HELICOPTER POSITION (SIMULATION) */}
        {simulationMode && dronePosition && (
          <Marker
            position={[dronePosition.lat, dronePosition.lon]}
            icon={createHelicopterIcon('#22c55e', telemetry?.attitude?.yaw || 0, 52)}
            zIndexOffset={2000}
          >
            <Popup>
              <div className="p-2 min-w-[200px]">
                <div className="font-semibold text-sm mb-2">
                  {selectedMission?.mission_name || 'Live Helicopter'}
                </div>
            ...
            <Tooltip permanent direction="top" offset={[0, -30]}>
              <div className="text-xs font-bold bg-white px-2 py-1 rounded shadow-lg border-2 border-green-500">
                <strong className="text-green-600">🚁 LIVE</strong>
                <br />
                <span className="text-gray-700">{telemetry?.battery?.remaining?.toFixed(0) || 0}% • {dronePosition.alt.toFixed(0)}m</span>
              </div>
            </Tooltip>
          </Marker>
        )}
      ...
      <div className="absolute bottom-4 left-4 z-[1000] bg-slate-900/95 backdrop-blur border border-slate-700 rounded-lg p-4 space-y-3 min-w-[280px]">
        ...
        <div className="space-y-2">
          <label className="flex items-center space-x-2 text-sm text-slate-300 cursor-pointer">
            <input
              type="checkbox"
              checked={showDemoDrones}
              onChange={(e) => setShowDemoDrones(e.target.checked)}
              className="w-4 h-4 rounded border-slate-600"
            />
            <span>Show Demo Drones</span>
          </label>
          ...
          {selectedMission && (
            <label className="flex items-center space-x-2 text-sm text-slate-300 cursor-pointer">
              <input
                type="checkbox"
                checked={showMissionPanel}
                onChange={(e) => setShowMissionPanel(e.target.checked)}
                className="w-4 h-4 rounded border-slate-600"
              />
              <span>Show Mission Panel</span>
            </label>
          )}
        </div>
        ...
        <div className="text-xs text-slate-500 pt-2 border-t border-slate-700">
          {selectedMission 
            ? `🚁 Mission Helicopter + ${demoDrones.length} Demo Drones` 
